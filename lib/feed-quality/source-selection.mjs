...
const collectableAccessMethods = new Set(["rss", "rss_or_html", "rss_or_api"]);
...
export function getCollectableFeedSources(sourcePool, { legacySources = [] } = {}) {
  ...
  for (const source of sourcePool.sources ?? []) {
    ...
    if (!collectableAccessMethods.has(source.access_method)) continue;
  ...
  for (const legacySource of legacySources) {
    ...
    selected.push({
      ...
      accessMethod: metadata?.access_method ?? "legacy"
    });
  }
